...
function refreshData(){
    ...
    contractBalance(function(result){
        ...
        balanceElem.textContent = 'There is ' + stripDecimals(rawStr, 3) + ' OKT in the mine';
    });
    
    web3.eth.getBalance(currentAddr).then(result => {
        rawStr = numberWithCommas(Number(web3.utils.fromWei(result)).toFixed(3));
        document.getElementById('userTrx').textContent = 'You have ' + stripDecimals(rawStr, 3) + ' OKT in your wallet';
    }).catch((err) => {
        console.log(err)
    });
    ...
    getMyMiners(function(miners){
        ...
        calculateEggBuySimple(web3.utils.toWei('0.1'),function(eggs){
            devFee(eggs,function(fee){
                sellsforexampledoc.textContent='0.1 OKT Hires ' + formatEggs(eggs-fee) + ' miners';
            });
        });
    });
    updateBuyPrice()
    ...
    prldoc.textContent=window.location.origin+"/OKTMiner/?ref="+currentAddr
